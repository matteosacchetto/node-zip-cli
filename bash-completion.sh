...
# e.g., __node-zip-cli_filter_opts "-a --option-1" "-a --option-1 -b --option-2" -> "-b --option-2"
__node-zip-cli_filter_opts() {
  local exclude=($1)
  local opts=($2)
  local filtered=()
...
# e.g., __node-zip-cli_sort_completions "-v" "-h" "--version" "--help" "a" "b" -> "a b -h -v --help --version"
#       compgen -W "-v -h --version --help a b" -- "" | __node-zip-cli_sort_completions > "a b -h -v --help --version"
__node-zip-cli_sort_completions() {
  local nonflags=()
  local flags=()
...
__node-zip-cli_completion() {
  ...
  local last_opt=$(__node-zip-cli_get_last_opt)
  ...
  case $subcommand in
    zip|tar|find)
      ...
      case $prev in
        ...
        ;;
        -k|--keep-parent)
          [[ $subcommand != "find" ]] && COMPREPLY=($(compgen -W "${OPTIONS[keep_parent]}" -- "$cur" | __node-zip-cli_sort_completions))
          return 0
        ;;
        -s|--symlink)
          case $subcommand in
            find)
              COMPREPLY=($(compgen -W "keep none" -- "$cur" | __node-zip-cli_sort_completions))
              return 0
            ;;
            zip|tar)
              COMPREPLY=($(compgen -W "${OPTIONS[symlink]} $(__node-zip-cli_filter_opts "-s --symlink" "${COMMANDS[$subcommand]}")" -- "$cur" | __node-zip-cli_sort_completions))
              return 0
            ;;
          esac
          return 0
        ;;
        --disable-ignore)
          COMPREPLY=($(compgen -W "${OPTIONS[disable_ignore]}  $(__node-zip-cli_filter_opts "--disable-ignore" "${COMMANDS[$subcommand]}")" -- "$cur" | __node-zip-cli_sort_completions))
          return 0
        ;;
        -d|--deflate)
          if [[ $subcommand == "zip" ]]; then
            COMPREPLY=($(compgen -W "${OPTIONS[compression_level]} $(__node-zip-cli_filter_opts "-d --deflate" "${COMMANDS[zip]}")" -- "$cur" | __node-zip-cli_sort_completions))
            return 0
          fi
        ;;
        -g|--gzip)
          if [[ $subcommand == "tar" ]]; then
            COMPREPLY=($(compgen -W "${OPTIONS[compression_level]} $(__node-zip-cli_filter_opts "-g --gzip" "${COMMANDS[tar]}")" -- "$cur" | __node-zip-cli_sort_completions))
            return 0
          fi
        ;;
        -t|--type)
  ...
  COMPREPLY=($(compgen -W "${COMMANDS[$subcommand]}" -- "$cur" | __node-zip-cli_sort_completions))
}
