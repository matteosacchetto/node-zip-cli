...
```bash
npm i https://github.com/matteosacchetto/node-zip-cli/releases/download/v0.2.5/node-zip-cli-0.2.5.tgz
```

Or install it globally with

```bash
npm i --location=global https://github.com/matteosacchetto/node-zip-cli/releases/download/v0.2.5/node-zip-cli-0.2.5.tgz
```
...
> **NOTE**: Up to the current version (0.2.5) the list of paths to ignore which are specified with this options are applied after default ignore paths (like `.git`) BUT before any .gitignore or .zipignore file. This means that paths you specify here could be overridden by the aforementioned files.
...
> **NOTE**: Up to the current version (0.2.5) the .zipignore builds on top of already existing .gitignore rules, so if you only want to ignore some additional files you **do not need** to copy paste the content of the .gitignore. Keep in mind that this behavior may change in the future
